    ...
    def validar_imagen(self):
        ...
        if imagen:
            ...
            if extension not in ['jpg', 'png', 'jpeg']:
                raise ValidationError('No se aceptan imágenes en este formato, debe anexarla en formato PNG/JPG/JPEG')
            if imagen.size > 102400:
                raise ValidationError('El tamaño de su imagen excede el límite asignado que es 100 KB')
        return imagen
# endregion
...
class PacienteForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = [
            'username', 'password', 'first_name', 'last_name', 'email',
            'tipo_doc', 'num_doc', 'genero', 'rh', 'telefono',
            'fecha_nacimiento', 'tipo_poblacion', 'ocupacion', 'eps', 'regimen'
        ]
        widgets = {
            ...
            'telefono': forms.TextInput(attrs={
                'placeholder': 'Ej. 3001234567', 'id': 'id_telefono', 'class': 'form-control'
            }),
            'password': forms.PasswordInput(),
            'email': forms.EmailInput(attrs={'placeholder': 'Ingrese su correo electrónico'}),
        }
        labels = {
            'num_doc': 'Número de documento', 'username': 'Nombre de Usuario',
            'password': 'Contraseña', 'first_name': 'Nombres', 'last_name': 'Apellidos',
            'email': 'Correo', 'telefono': 'Teléfono', 'fecha_nacimiento': 'Fecha de nacimiento',
            'tipo_poblacion': 'Tipo de población', 'ocupacion': 'Ocupación', 'eps': 'EPS',
            'regimen': 'Régimen de afiliación', 'tipo_doc': 'Tipo de documento',
            'genero': 'Género', 'rh': 'RH'
        }
...
class AdministradorForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            'fecha_nacimiento': forms.DateInput(attrs={
                'class': 'datepicker', 'type': 'text', 'placeholder': 'Seleccione la fecha de nacimiento'
            }),
            'telefono': forms.TextInput(attrs={
                'placeholder': 'Ej. 3001234567', 'id': 'telefono-input', 'class': 'form-control'
            }),
            'password': forms.PasswordInput(),
            'email': forms.EmailInput(attrs={'placeholder': 'Ingrese su correo electrónico'}),
        ...
        labels = {
            ...
            'tipo_doc': 'Tipo de documento', 'num_doc': 'Número de documento',
            'genero': 'Género', 'rh': 'RH', 'telefono': 'Teléfono',
            'fecha_nacimiento': 'Fecha de nacimiento', 'tipo_poblacion': 'Tipo de población',
            'ocupacion': 'Ocupación', 'eps': 'EPS', 'rol_acceso': 'Rol de Acceso',
...
class MedicoForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = [
            ...
            'numero_registro_profesional', 'licencia_certificacion', 'fecha_contratacion'
        ]
        widgets = {
            'fecha_contratacion': forms.DateInput(attrs={'class': 'datepicker', 'placeholder': 'Fecha de contratación'}),
            'fecha_nacimiento': forms.DateInput(attrs={'class': 'datepicker', 'placeholder': 'Fecha de nacimiento'}),
            'telefono': forms.TextInput(attrs={
                'placeholder': 'Ej. 3001234567', 'id': 'telefono-input', 'class': 'form-control'
            }),
        }
...
class ConsultaForm(forms.ModelForm):
    diagnostico_principal = forms.ModelChoiceField(
        ...
        widget=forms.Select(attrs={'class': 'form-control select2', 'data-placeholder': 'Seleccione un diagnóstico principal'}),
        required=True
    )
    diagnostico_relacionado = forms.ModelChoiceField(
        queryset=TablaReferenciaCIE10.objects.all(),
        empty_label="Seleccione un diagnóstico relacionado",
        widget=forms.Select(attrs={'class': 'form-control select2', 'data-placeholder': 'Seleccione un diagnóstico relacionado'}),
        required=False
    )
    ...
    class Meta:
        ...
        labels = {
            'especialidad': 'Especialidad', 'tratamiento': 'Tratamiento',
...
class PerfilPacienteForm(forms.ModelForm):
    vida_sexual = forms.ChoiceField(
        label='Vida sexual',
        choices=[('', 'Selecciona una opción')] + PerfilPaciente.OPCIONES_VIDA_SEXUAL
    )

    class Meta:
        model = PerfilPaciente
        fields = [
            'paciente', 'tratamiento', 'vida_sexual', 'ciclo_mestrual',
            'sustancias_psicotivas', 'habitos_alimenticios', 'consumo_alcohol',
            'habito_sueño', 'antecedentes_personales'
        ]
        widgets = {
...
class HorarioMedicoForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'dia_semana':  forms.Select(attrs={'class': 'form-control select2'}),
        }
# endregion
...
class CitaForm(forms.ModelForm):
    estado_cita = forms.ChoiceField(
        ...
        choices=[('', 'Selecciona una opción')] + Cita.OPCIONES_ESTADO_CITA
    )
    ...
    class Meta:
        ...
        widgets = {
            ...
            'medico':     forms.Select(attrs={'class': 'form-control', 'readonly': 'readonly'}),
            'paciente':   forms.Select(attrs={'class': 'form-control', 'readonly': 'readonly'}),
        }
# endregion
...
class CertificadoIncapacidadForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = [
            'dias_incapacidad', 'motivo_incapacidad', 'fecha_inicio', 'fecha_fin',
        ...
        widgets = {
            ...
            'fecha_inicio': forms.SelectDateWidget(attrs={'class': 'form-control'}, years=range(2025, 2030)),
            'fecha_fin':    forms.SelectDateWidget(attrs={'class': 'form-control'}, years=range(2025, 2030)),
...
class RecetaMedicaForm(forms.ModelForm):
    ...
    class Meta:
        model = RecetaMedica
        fields = [
            'medicamento', 'concentracion', 'duracion', 'cantidad', 'via_administracion',
            'diagnostico_principal', 'diagnostico_relacionado', 'intervalo',
            'recomendaciones', 'indicaciones', 'fecha_medicado',
        ]
        widgets = {
            ...
            'fecha_medicado':    forms.DateInput(attrs={'class': 'form-control', 'type': 'date'}),
        }
# endregion
...
class OrdenMedicaForm(forms.ModelForm):
    ...
    class Meta:
        model = OrdenMedica
        fields = [
            'cups', 'especialidad_referido', 'cantidad', 'diagnostico_principal',
            'diagnostico_relacionado', 'motivo', 'vigencia'
        ]
        widgets = {
            ...
            'vigencia':            forms.SelectDateWidget(attrs={'class': 'form-control'}, years=range(2025, 2030)),
        }
# endregion
...
class RegistroClinicoForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'ultima_atencion': forms.DateInput(attrs={'type': 'date', 'readonly': 'readonly'}),
            'tratamiento':     forms.Textarea(attrs={'rows': 4}),
...
class DatoAntropometricoForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'altura_decimal':       forms.NumberInput(attrs={'placeholder': 'Altura en metros (ej: 1.75)'}),
            'peso':                 forms.NumberInput(attrs={'placeholder': 'Peso en kg'}),
            'indice_masa_corporal': forms.NumberInput(attrs={'placeholder': 'Se calcula automáticamente', 'readonly': 'readonly'}),
        }
# endregion